...
- `world_start`: `schema`, `seq`, `word_bytes`, `actor_capacity`,
  ...
  `mailbox_byte_limit`, `monitor_limit`, and `timer_limit`.
...
- `send_rejected`: `seq`, `sender`, `receiver`, and a bounded `reason` enum.
  Its values are `missing`, `quota`, `unsupported`, and `resource`. It never
...
`world_start.schema` rather than interpreting it as v2.
...
- `timer`: `seq`, owner `actor`, decimal-string `timer` identity, `action`
  (`created`, `expired`, `consumed`, `cancelled`), and decimal-string
  `deadline_ns`. Strings preserve 64-bit precision in JavaScript consumers.
- `timer_cleanup`: `seq`, `actor`, and `count` of timers discarded on actor or
  world teardown. Consumers remove all remaining timers for that actor.

Creation is recorded after commit. Expiry records batched scheduler promotion;
await/cancel may directly consume an already-due timer before promotion, so an
`expired` event is optional before `consumed`. Deadline ties use registration
order. A complete world's cleanup events precede its footer. Clock-dependent
runs require the same external time inputs for deterministic replay; v2 does
not promise identical traces across real-time runs. No clock reads occur solely
for tracing. Schema v1 remains readable by the bundled viewer.
